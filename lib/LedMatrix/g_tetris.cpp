...
static void drawFigure(byte figure, byte angle, byte x, byte y, uint32_t color);
static void redrawFigure(int8_t clr_ang, int8_t clr_pos, int8_t clr_height);
static boolean checkArea(int8_t check_type);
static void newGameTetris();
static void stepRight();
static void stepLeft();
static void fixFigure();
static void gameOverTetris();
...
static void checkAndClear()
{
  ...
  { // чисти чисти пока не будет чисто!
    linesToClear = 0;
    ...
    {                   // сканируем по высоте
      ...
      { // проходимся по строкам
        ...
        {                    // если хоть один пиксель чёрный
          full_flag = false; // считаем строку неполной
        }
      }
      if (full_flag)
      {                     // если нашлась заполненная строка
...
static boolean checkArea(int8_t check_type)
{
  ...
  for (byte i = 0; i < 4; i++)
  {
    ...
    {
      if (X + 1 > WIDTH - 1)
        flag = false; // смотрим следующий справа
      uint32_t getColor;
      if (Y < HEIGHT)
    ...
    {
      if (X - 1 < 0)
        flag = false; // смотрим следующий слева
      uint32_t getColor;
      if (Y < HEIGHT)
        getColor = getPixColorXY(X - offset, Y);
      if (getColor != color && getColor != 0x000000)
