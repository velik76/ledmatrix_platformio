...
static bool gamemodeFlag = false; // флаг: текущий режим - игра

static void CarveMaze(char *maze, int width, int height, int x, int y);
static void GenerateMaze(char *maze, int width, int height);
static void SolveMaze(char *maze, int width, int height);
static void newGameMaze();
static void movePlayer(int8_t nowX, int8_t nowY, int8_t prevX, int8_t prevY);
...
void smartMaze()
{
  ...
  int attempt;
  while (sum < MIN_PATH)
  { // пока длина пути меньше заданной
    attempt++;
    ...
    { // перебор всех пикселей (элементов массива maze)
      for (byte x = 0; x < MAZE_WIDTH; x++)
      {
        if (maze[y * MAZE_WIDTH + x] == 2)
          sum++; // в массиве maze пиксель "правильного пути" имеет значение 2
      }
    }
    if (MAX_STRAIGHT > 0)
    {
      ...
      line = 0;
      for (byte y = 0; y < MAZE_HEIGHT; y++)
        if (maze[y * MAZE_WIDTH + MAZE_WIDTH - 2] == 2)
          line++;
      if (line > MAX_STRAIGHT)
        sum = 0;

      line = 0;
      for (byte x = 0; x < MAZE_WIDTH; x++)
        if (maze[MAZE_WIDTH + x] == 2)
          line++;
      if (line > MAX_STRAIGHT)
        sum = 0;
    }
...
void makeHoles()
{
  ...
  byte attempt;
  while (holes < HOLES)
  {            // пока текущее число дыр меньше заданного
    attempt++; // прибавляем число попыток
    if (attempt > 200)
      break; // если совершено более 200 попыток, завершить алгоритм (если зависнет)
    ...
    {                               // (это чтобы не дырявить угловые стенки)
...
static void SolveMaze(char *maze, int width, int height)
{
  ...
  while (x != width - 2 || y != height - 2)
  {
    ...
    {        // если решатель не может найти решение (maxSolves в 5 раз больше числа клеток лабиринта)
