...
// **************** НАСТРОЙКИ RUNNER ****************
// 1 пиксель = 10 мм
#define POP_SPEED 520 // скорость подскока (в мм/с)
#define GRAVITY 1400  // ускорение падения (в мм/с2)
#define DT 20         // период интегрирования (мс)
#define OBST_SPEED 40 // скорость препятствий
...
Timer runnerTimer(DT);
Timer obstTimer(OBST_SPEED);
...
void runnerRoutine(bool init)
{
  ...
  if (gameDemo)
  {
    if (getPixColorXY(DEMO_JUMP, 0) == GLOBAL_COLOR_2)
