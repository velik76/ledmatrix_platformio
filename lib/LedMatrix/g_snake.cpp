...
static void snakeDemo()
{
  ...
  // ищем яблоко
  if (headX == appleX)
  { // яблоко на одной линии по вертикали
    if (headY < appleY)
      buttons = 0;
    if (headY > appleY)
      buttons = 2;
  ...
  if (headY == appleY)
  { // яблоко на одной линии по горизонтали
    if (headX < appleX)
      buttons = 1;
    if (headX > appleX)
      buttons = 3;
      ...
      || nextY > HEIGHT - 1)
  {
    ...
    if (vectorX > 0)
      buttons = 2;
    if (vectorX > 0 && headY == 0)
      buttons = 0;

    if (vectorX < 0 && headY == HEIGHT - 1)
      buttons = 2;
    if (vectorX < 0)
      buttons = 0;
    ...
    if (vectorY > 0)
      buttons = 1;
    if (vectorY > 0 && headX == WIDTH - 1)
      buttons = 3;

    if (vectorY < 0 && headX == 0)
      buttons = 1;
    if (vectorY < 0)
      buttons = 3;
  }
...
void snakeGame(bool init)
{
  ...
  if (vectorX > 0)
    buttVector[snakeLength] = 0;
  else if (vectorX < 0)
    buttVector[snakeLength] = 1;
  if (vectorY > 0)
    buttVector[snakeLength] = 2;
  else if (vectorY < 0)
    buttVector[snakeLength] = 3;
  ...
  if (!pizdetc)
  {
    ...
    case 3:
      ...
      break;
    case 4:
      missDelete = true; // 4 значит не стирать!
      break;
    }
